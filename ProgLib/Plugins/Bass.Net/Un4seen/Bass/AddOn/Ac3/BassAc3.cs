...
namespace Un4seen.Bass.AddOn.Ac3
{
	...
	public sealed class BassAc3
	{
		...
		private static extern int BASS_AC3_StreamCreateFileUnicode([MarshalAs(UnmanagedType.Bool)] bool mem, [MarshalAs(UnmanagedType.LPWStr)] [In] string file, long offset, long length, BASSFlag flags);
		...
		public static int BASS_AC3_StreamCreateFile(string file, long offset, long length, BASSFlag flags)
		{
			...
			return BassAc3.BASS_AC3_StreamCreateFileUnicode(false, file, offset, length, flags);
		}
		...
		{
			return BassAc3.BASS_AC3_StreamCreateFileMemory(true, memory, offset, length, flags);
		}
		...
		[DllImport("bass_ac3")]
		private static extern int BASS_AC3_StreamCreateURLAscii([MarshalAs(UnmanagedType.LPStr)] [In] string url, int offset, BASSFlag flags, DOWNLOADPROC proc, IntPtr user);

		[DllImport("bass_ac3")]
		private static extern int BASS_AC3_StreamCreateURLUnicode([MarshalAs(UnmanagedType.LPWStr)] [In] string url, int offset, BASSFlag flags, DOWNLOADPROC proc, IntPtr user);

		public static int BASS_AC3_StreamCreateURL(string url, int offset, BASSFlag flags, DOWNLOADPROC proc, IntPtr user)
		{
			flags |= BASSFlag.BASS_UNICODE;
			int num = BassAc3.BASS_AC3_StreamCreateURLUnicode(url, offset, flags, proc, user);
			if (num == 0)
			{
				...
				num = BassAc3.BASS_AC3_StreamCreateURLAscii(url, offset, flags, proc, user);
			}
			return num;
		}
		...
		{
			return Utils.LoadLib("bass_ac3", ref BassAc3._myModuleHandle);
		}

		public static bool LoadMe(string path)
		{
			return Utils.LoadLib(Path.Combine(path, "bass_ac3"), ref BassAc3._myModuleHandle);
		}

		public static bool FreeMe()
		{
			return Utils.FreeLib(ref BassAc3._myModuleHandle);
		}
