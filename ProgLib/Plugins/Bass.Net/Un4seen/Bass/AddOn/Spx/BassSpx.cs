...
namespace Un4seen.Bass.AddOn.Spx
{
	...
	public sealed class BassSpx
	{
		...
		private static extern int BASS_SPX_StreamCreateFileUnicode([MarshalAs(UnmanagedType.Bool)] bool mem, [MarshalAs(UnmanagedType.LPWStr)] [In] string file, long offset, long length, BASSFlag flags);
		...
		public static int BASS_SPX_StreamCreateFile(string file, long offset, long length, BASSFlag flags)
		{
			...
			return BassSpx.BASS_SPX_StreamCreateFileUnicode(false, file, offset, length, flags);
		}
		...
		{
			return BassSpx.BASS_SPX_StreamCreateFileMemory(true, memory, offset, length, flags);
		}
		...
		[DllImport("bass_spx")]
		private static extern int BASS_SPX_StreamCreateURLAscii([MarshalAs(UnmanagedType.LPStr)] [In] string url, int offset, BASSFlag flags, DOWNLOADPROC proc, IntPtr user);

		[DllImport("bass_spx")]
		private static extern int BASS_SPX_StreamCreateURLUnicode([MarshalAs(UnmanagedType.LPWStr)] [In] string url, int offset, BASSFlag flags, DOWNLOADPROC proc, IntPtr user);

		public static int BASS_SPX_StreamCreateURL(string url, int offset, BASSFlag flags, DOWNLOADPROC proc, IntPtr user)
		{
			flags |= BASSFlag.BASS_UNICODE;
			int num = BassSpx.BASS_SPX_StreamCreateURLUnicode(url, offset, flags, proc, user);
			if (num == 0)
			{
				...
				num = BassSpx.BASS_SPX_StreamCreateURLAscii(url, offset, flags, proc, user);
			}
			return num;
		}
		...
		{
			return Utils.LoadLib("bass_spx", ref BassSpx._myModuleHandle);
		}

		public static bool LoadMe(string path)
		{
			return Utils.LoadLib(Path.Combine(path, "bass_spx"), ref BassSpx._myModuleHandle);
		}

		public static bool FreeMe()
		{
			return Utils.FreeLib(ref BassSpx._myModuleHandle);
		}
