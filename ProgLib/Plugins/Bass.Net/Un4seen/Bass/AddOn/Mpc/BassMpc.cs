...
namespace Un4seen.Bass.AddOn.Mpc
{
	...
	public sealed class BassMpc
	{
		...
		private static extern int BASS_MPC_StreamCreateFileUnicode([MarshalAs(UnmanagedType.Bool)] bool mem, [MarshalAs(UnmanagedType.LPWStr)] [In] string file, long offset, long length, BASSFlag flags);
		...
		public static int BASS_MPC_StreamCreateFile(string file, long offset, long length, BASSFlag flags)
		{
			...
			return BassMpc.BASS_MPC_StreamCreateFileUnicode(false, file, offset, length, flags);
		}
		...
		{
			return BassMpc.BASS_MPC_StreamCreateFileMemory(true, memory, offset, length, flags);
		}
		...
		private static extern int BASS_MPC_StreamCreateURLAscii([MarshalAs(UnmanagedType.LPStr)] [In] string url, int offset, BASSFlag flags, DOWNLOADPROC proc, IntPtr user);

		[DllImport("bass_mpc", EntryPoint = "BASS_MPC_StreamCreateURL")]
		private static extern int BASS_MPC_StreamCreateURLUnicode([MarshalAs(UnmanagedType.LPWStr)] [In] string url, int offset, BASSFlag flags, DOWNLOADPROC proc, IntPtr user);

		public static int BASS_MPC_StreamCreateURL(string url, int offset, BASSFlag flags, DOWNLOADPROC proc, IntPtr user)
		{
			flags |= BASSFlag.BASS_UNICODE;
			int num = BassMpc.BASS_MPC_StreamCreateURLUnicode(url, offset, flags, proc, user);
			if (num == 0)
			{
				...
				num = BassMpc.BASS_MPC_StreamCreateURLAscii(url, offset, flags, proc, user);
			}
			return num;
		}
		...
		{
			return Utils.LoadLib("bass_mpc", ref BassMpc._myModuleHandle);
		}

		public static bool LoadMe(string path)
		{
			return Utils.LoadLib(Path.Combine(path, "bass_mpc"), ref BassMpc._myModuleHandle);
		}

		public static bool FreeMe()
		{
			return Utils.FreeLib(ref BassMpc._myModuleHandle);
		}
