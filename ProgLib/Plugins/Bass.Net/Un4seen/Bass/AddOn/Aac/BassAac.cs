...
namespace Un4seen.Bass.AddOn.Aac
{
	...
	public sealed class BassAac
	{
		...
		private static extern int BASS_AAC_StreamCreateFileUnicode([MarshalAs(UnmanagedType.Bool)] bool mem, [MarshalAs(UnmanagedType.LPWStr)] [In] string file, long offset, long length, BASSFlag flags);
		...
		public static int BASS_AAC_StreamCreateFile(string file, long offset, long length, BASSFlag flags)
		{
			...
			return BassAac.BASS_AAC_StreamCreateFileUnicode(false, file, offset, length, flags);
		}
		...
		{
			return BassAac.BASS_AAC_StreamCreateFileMemory(true, memory, offset, length, flags);
		}
		...
		private static extern int BASS_AAC_StreamCreateURLAscii([MarshalAs(UnmanagedType.LPStr)] [In] string url, int offset, BASSFlag flags, DOWNLOADPROC proc, IntPtr user);
		...
		private static extern int BASS_MP4_StreamCreateFileUnicode([MarshalAs(UnmanagedType.Bool)] bool mem, [MarshalAs(UnmanagedType.LPWStr)] [In] string file, long offset, long length, BASSFlag flags);

		public static int BASS_MP4_StreamCreateFile(string file, long offset, long length, BASSFlag flags)
		{
			flags |= BASSFlag.BASS_UNICODE;
			return BassAac.BASS_MP4_StreamCreateFileUnicode(false, file, offset, length, flags);
		}
		...
		{
			return BassAac.BASS_MP4_StreamCreateFileMemory(true, memory, offset, length, flags);
		}
		...
		{
			return Utils.LoadLib("bass_aac", ref BassAac._myModuleHandle);
		}

		public static bool LoadMe(string path)
		{
			return Utils.LoadLib(Path.Combine(path, "bass_aac"), ref BassAac._myModuleHandle);
		}

		public static bool FreeMe()
		{
			return Utils.FreeLib(ref BassAac._myModuleHandle);
		}
