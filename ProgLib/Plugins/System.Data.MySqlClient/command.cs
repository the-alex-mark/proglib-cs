  ...
  public sealed partial class MySqlCommand : ICloneable, IDisposable
  {
    ...
    private void EnsureCommandIsReadOnly(string sql)
    {
      ...
      if (!sql.StartsWith("select") && !sql.StartsWith("show"))
        Throw(new MySqlException(MySqlResources.ReplicatedConnectionsAllowOnlyReadonlyStatements));
    ...
    {
      if (caller != null)
        Throw(new MySqlException(MySqlResources.UnableToStartSecondAsyncOp));
    ...
    private bool AddCallStatement(string query)
    {
      ...
      if (keywords == null)
        keywords = new List<string>(ProgLib.Properties.Resources.keywords.Replace("\r", "").Split('\n'));
