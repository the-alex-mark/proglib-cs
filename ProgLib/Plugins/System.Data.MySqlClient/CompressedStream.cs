...
namespace System.Data.MySqlClient
{
  ...
  internal class CompressedStream : Stream
  {
    ...
    {
      throw new NotSupportedException(MySqlResources.CSNoSetLength);
    }
    ...
    {
      if (buffer == null)
        throw new ArgumentNullException("buffer", MySqlResources.BufferCannotBeNull);
      if (offset < 0 || offset >= buffer.Length)
        throw new ArgumentOutOfRangeException("offset", MySqlResources.OffsetMustBeValid);
      if ((offset + count) > buffer.Length)
        throw new ArgumentException(MySqlResources.BufferNotLargeEnough, "buffer");
