...
namespace System.Data.MySqlClient
{
  ...
  public sealed partial class MySqlDataReader : IDisposable
  {
    ...
    public override Guid GetGuid(int i)
    {
      ...
      Throw(new MySqlException(MySqlResources.ValueNotSupportedForGuid));
    ...
    {
      if (!isOpen)
        Throw(new MySqlException(MySqlResources.NextResultIsClosed));
      ...
      catch (MySqlException ex)
      {
        if (ex.IsFatal)
          connection.Abort();
        if (ex.Number == 0)
          throw new MySqlException(MySqlResources.FatalErrorReadingResult, ex);
    ...
    public override bool Read()
    {
      ...
      catch (MySqlException ex)
      {
        ...
        throw new MySqlException(MySqlResources.FatalErrorDuringRead, ex);
      }
    }
    ...
    private IMySqlValue GetFieldValue(int index, bool checkNull)
    {
      if (index < 0 || index >= FieldCount)
        Throw(new ArgumentException(MySqlResources.InvalidColumnOrdinal));
